...
const UserProfile = (props) => {
  ...
  className = { styles.aside } >
    < span;
  className = { styles.headerAside } > Select;
  Option < /span>
  < div;
  ...
  onClick = {();
=>
  changeProfileModeView(CONSTANTS.USER_INFO_MODE);
}>
  UserInfo
  < /div>;
  {
    ...
    onClick = {();
  =>
    changeProfileModeView(CONSTANTS.CASHOUT_MODE);
  }>
    Cashout < /div>}
    < /div>
    < /div>;
    ...
    <
      span;
      className = { styles.notMoney } > There;
      is;
      no;
      money;
      on;
