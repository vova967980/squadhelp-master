...
const StartContestPage = (props) => {
  ...
  const setBundle = (bundleStr) => {
    ...
    for (let i = 0; i < array.length; i++) {
      bundleList[ array[ i ] ] = i === array.length - 1 ? 'payment' : array[ i +
      1 ];
    }
  ...
  return (
    ...
    < Header / >
    < div;
  className = { styles.startContestHeader } >
    < div;
  className = { styles.startContestInfo } >
    < h2 >
    START;
  A;
  CONTEST
  ...
  < /div>
  < div;
  className = { styles.baseBundleContainer } >
    < div;
  className = { styles.infoBaseBundles } >
    < span;
  className = { styles.headerInfo } > Our;
  Most;
  Popular < span > Categories < /span></s;
  pan >
  < span;
  className = { styles.info } > Pick;
  from;
  our;
  most;
  popular;
  ...
  < /div>
  < div;
  className = { styles.combinedBundles } >
    < div;
  className = { styles.infoCombinedBundles } >
    < span;
  className = { styles.headerInfo } > Save;
  With;
  Our;
  Bundle;
  Packages < /span>
  < span;
  className = { styles.info } > Launch;
  multiple;
  contests;
  and;
  pay;
