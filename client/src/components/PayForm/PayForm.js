...
const PayForm = (props) => {
    ...
    return (
        <div className={styles.payFormContainer}>
            ...
            <form id='myForm' className={styles.formContainer} onSubmit={handleSubmit(pay)}>
            ...
            {isPayForOrder && <div className={styles.totalSum}><span>Total: $100.00</span></div>}
            <div className={styles.buttonsContainer}>
                <button form='myForm' className={styles.payButton} type='submit'>
                    <span>{isPayForOrder ? 'Pay Now' : 'CashOut'}</span></button>
                {isPayForOrder &&
                <div onClick={() => props.back()} className={styles.backButton}><span>Back</span></div>}
            </div>
        </div>
    )
...
const mapStateToProps = (state) => {
    ...
    const {name, number, cvc, expiry} = selector(state, 'name', 'number', 'cvc', 'expiry');
...
export default connect(mapStateToProps, mapDispatchToProps)(reduxForm({
    form: 'payForm',
    validate: customValidator(isPayForOrder ? Schems.PaymentSchema : Schems.CashoutSchema)
})(PayForm));
