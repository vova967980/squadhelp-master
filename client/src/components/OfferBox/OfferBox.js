...
const OfferBox = (props) => {
  ...
  const resolveOffer = () => {
    confirmAlert({
      ...
      buttons: [
        {
          label: 'Yes',
          onClick: () => props.setOfferStatus(props.data.User.id, props.data.id,
            'resolve'),
        },
        {
          label: 'No',
  ...
  className = { styles.userInfo } >
    < div;
  className = { styles.creativeInfoContainer } >
    < img;
  src = { avatar === 'anon.png'
  ...
  className = { styles.nameAndEmail } >
    ...
    < /div>
    < div;
  className = { styles.creativeRating } >
    < span;
  className = { styles.userScoreLabel } > Creative;
  Rating < /span>
  < Rating;
  ...
  < /div>
  < div;
  className = { styles.responseConainer } >
  {
...
<
  ...
  onClick = {();
=>
  props.changeShowImage({ imagePath: data.fileName, isShowOnFull: true });
}
...
<
  span;
  className = { styles.response } > { data.text } < /span>;
}
  {
  ...
  <
    /div>;
    {
      props.needButtons(data.status) && < div;
      className = { styles.btnsContainer } >
        < div;
      onClick = { resolveOffer };
      className = { styles.resolveBtn } > Resolve < /div>
        < div;
      onClick = { rejectOffer };
