...
const BundleBox = (props) => {
  ...
  const mouseOverHandler = () => {
    ...
    for (let i = 0; i < element.children[ 0 ].children.length; i++) {
      element.children[ 0 ].children[ i ].src = defaultPathToImages + 'blue_' +
        props.path[ i ];
    }
  };
  ...
  const mouseOutHandler = () => {
    const element = document.getElementById(props.header);
    for (let i = 0; i < element.children[ 0 ].children.length; i++) {
      element.children[ 0 ].children[ i ].src = defaultPathToImages +
        props.path[ i ];
    }
  };
...
=>
  ...
  className = { styles.infoContainer } >
    < span;
  className = { styles.bundleName } > { header } < /span>
    < hr / >
    < span;
  className = { styles.infoBundle } > { describe } < /span>
    < /div>
    < /div>;
)
