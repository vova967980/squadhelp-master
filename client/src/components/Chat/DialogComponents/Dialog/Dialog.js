...
class Dialog extends React.Component{
  ...
  renderMainDialog = () => {
    ...
    messages.forEach((message, i) => {
      ...
      < div;
      key = { i };
      className = { className (userId === message.sender
        ? styles.ownMessage
        : styles.message;
    )
    }>
    <
      ...
      < span;
      className = { styles.messageTime } > { moment (message.createdAt
    ).
      format('HH:mm');
    }<
      /span>
  ...
  blockMessage = () => {
    ...
    return (
      < span;
    className = { styles.messageBlock } > { message } < /span>;
  )
  };
  ...
  <
    ...
    />;
    {
      ( chatData && chatData.blackList.includes(true) ) ? this.blockMessage() :
    <
      ChatInput / >
    }
  <
