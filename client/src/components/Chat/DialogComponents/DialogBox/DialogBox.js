...
<
  ...
  className = { styles.infoContainer } >
    < div;
  className = { styles.interlocutorInfo } >
    < span;
  className = { styles.interlocutorName } > { interlocutor.firstName } < /span>
    < span;
  className = { styles.interlocutorMessage } > { text } < /span>
    < /div>
    < div;
  className = { styles.buttonsContainer } >
    < span;
  className = { styles.time } > { getTimeStr (createAt) } < /span>
    < i;
  onClick = {(event);
=>
  changeFavorite({
...
=>
  ...
  className = { classNames ({ [ 'fas fa-user-lock' ]:
  !isBlocked, ['fas fa-unlock'];
:
  isBlocked;
})
}
