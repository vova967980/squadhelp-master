...
class CustomerDashboard extends React.Component{
  ...
  getContests = () => {
    this.props.getContests(
      { limit: 8, contestStatus: this.props.customerFilter });
  };
  ...
  =>
    ...
    className = {
        classNames ({
          [ styles.activeFilter ]: CONSTANTS.CONTEST_STATUS_ACTIVE ===
      customerFilter,
      [styles.filter];
  :
  ...
  =>
    ...
    className = {
        classNames ({
          [ styles.activeFilter ]: CONSTANTS.CONTEST_STATUS_FINISHED ===
      customerFilter,
      [styles.filter];
  :
  ...
  =>
    ...
    className = {
        classNames ({
          [ styles.activeFilter ]: CONSTANTS.CONTEST_STATUS_PENDING ===
      customerFilter,
      [styles.filter];
  :
    ...
    < /div>
    < div;
    className = { styles.contestsContainer } >
      {
        error ?
      < TryAgain;
    getData = { this.tryToGetContest() };
    />;
  :
  <
    ContestsContainer;
