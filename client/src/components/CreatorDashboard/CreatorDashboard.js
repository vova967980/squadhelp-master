...
class CreatorDashboard extends React.Component{
  ...
  renderSelectType = () => {
    ...
    types.forEach((el, i) => !i || array.push( < option
    key = { i -1 };
    value = { el } > { el } < /option>));;
    return (
      < select;
  ...
  renderIndustryType = () => {
    ...
    value = { null } > Choose;
    industry < /option>);;
    industry.forEach((industry, i) => array.push( < option
    key = { i +1 };
  ...
  componentDidMount () {
    ...
    if (this.parseUrlForParams(this.props.location.search) &&
  ...
  tryLoadAgain = () => {
    ...
    this.props.getContests(
      { limit: 8, offset: 0, ...this.getPredicateOfRequest() });
  };
  ...
  render () {
    ...
    className = { styles.filterContainer } >
      < span;
    className = { styles.headerFilter } > Filter;
    Results < /span>
    < div;
  ...
  =>
    this.changePredicate(
      { name: 'ownEntries', value: !creatorFilter.ownEntries });
  }
    className = { classNames (styles.myEntries
    {
      [styles.activeMyEntries];
    :
